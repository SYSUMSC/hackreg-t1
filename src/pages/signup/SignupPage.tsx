...
const getSubmitHandler = (
  updateInitialFormData: React.Dispatch<React.SetStateAction<FormData | null>>,
  updateErroredAction: React.Dispatch<React.SetStateAction<ErroredAction>>
  ...
  Promise.race<Promise<Response>>([
    ...
    .catch(error => {
      ...
      if (error instanceof FormDataError) {
        ...
        error.names.forEach(name =>
          Object.assign(erroredFormItems, { [name]: '' })
        );
...
const handleCancel = (
  values: FormData,
  updateInitialFormData: React.Dispatch<React.SetStateAction<FormData | null>>,
  updateErroredAction: React.Dispatch<
    React.SetStateAction<ErroredAction | null>
  >,
...
const handleFetch = (
  updateErrorMsg: React.Dispatch<React.SetStateAction<string | null>>,
  updateInitialFormData: React.Dispatch<React.SetStateAction<FormData | null>>
) => {
  Promise.race<Promise<Response>>([
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error('请求超时，请刷新重试')), 8000)
    ),
    fetch('/signup/fetch', {
      // TODO: change it to /backend/signup
      method: 'GET',
    ...
    .then(([ok, data]) => {
      // data here will be either form data object or error message object
      if (ok) {
...
const emptyFormData: FormData = {
  confirmed: false,
  form: { teamInfo: { name: '', description: '' }, memberInfo: [] }
};
...
const SignupPage: FunctionComponent = () => {
  ...
  const [initialFormData, updateInitialFormData] = useState<FormData | null>(
    null
  );
  ...
  if (!initialFormData) {
    return (
      <div className="signup-page-container spinner-container content-fit-viewport">
        ...
        {errorMsg ? (
          <span className="font-weight-light content-error-msg">
            {errorMsg}
          </span>
        ) : null}
      </div>
                ...
                <Col>
                  <Form.Group id="confirmed">
                    ...
                    <Form.Check
                      type="checkbox"
                      disabled={
                        formik.isSubmitting || formik.initialValues.confirmed
                      }
                      label="确认报名并锁定表单"
                    ...
                    <Form.Text className="text-muted font-weight-light">
                      确认报名并提交表单后，报名才算作有效。报名结束前可以任意取消报名来修改表单。
                    </Form.Text>
                  </Form.Group>
                </Col>
              </Row>
