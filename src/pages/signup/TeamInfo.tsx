...
const TeamInfo: FunctionComponent<{ formik: FormikProps<FormData> }> = ({ formik }) => {
    return (<>
        <h4>队伍信息</h4>
        <Form.Group controlId="form.teamInfo.name">
            ...
            <Form.Control
                type="text"
                isInvalid={!!formik.touched?.form?.teamInfo?.name && !!formik.errors?.form?.teamInfo?.name}
                maxLength={20}
        ...
        <Form.Group controlId="form.teamInfo.description">
            ...
            <Form.Control
                as="textarea"
                rows="5"
                isInvalid={!!formik.touched?.form?.teamInfo?.description && !!formik.errors?.form?.teamInfo?.description}
                maxLength={50}
                disabled={formik.isSubmitting || formik.initialValues.confirmed}
