...
const FormItem: FunctionComponent<{
  ...
  controlId: string;
  colCss?: string;
}> = ({ children, title, controlId, colCss }) => (
...
const MemberInfoCard: FunctionComponent<{
    ...
    <Card {...(!!erroredItems && !!touchedItems && { border: 'danger' })}>
      ...
      <Accordion.Collapse eventKey={`form.memberName-${index}`}>
        <Card.Body>
          ...
          <FormItem
            title="性别"
            colCss="radio-container"
            controlId={`form.memberInfo[${index}].gender`}
          >
            <Form.Control
              as="select"
              disabled={isSubmitting || formik.initialValues.confirmed}
          ...
          <FormItem
            title="手机号码"
            controlId={`form.memberInfo[${index}].phone`}
          >
            <Form.Control
              type="text"
          ...
          <FormItem
            title="衣服尺码"
            controlId={`form.memberInfo[${index}].size`}
          >
            <Form.Control
              as="select"
          ...
          <FormItem
            title="学历"
            controlId={`form.memberInfo[${index}].education`}
          >
            <Form.Control
              as="select"
          ...
          <FormItem
            title="专业"
            controlId={`form.memberInfo[${index}].profession`}
          >
            <Form.Control
              type="text"
              isInvalid={
                !!touchedItems?.profession && !!erroredItems?.profession
              }
              maxLength={20}
              disabled={isSubmitting || formik.initialValues.confirmed}
          ...
          <FormItem
            title="个人经历"
            controlId={`form.memberInfo[${index}].experience`}
          >
            <Form.Control
              as="textarea"
              rows="5"
              isInvalid={
                !!touchedItems?.experience && !!erroredItems?.experience
              }
              maxLength={100}
              disabled={isSubmitting || formik.initialValues.confirmed}
...
const MemberInfo: FunctionComponent<{ formik: FormikProps<FormData> }> = ({
  formik
}) => {
      ...
      <Accordion>
        ...
        {memberInfo.length < 6 ? (
          <>
            <Card
              key="addMember"
              {...(!!formik.errors?.form?.memberInfo &&
                formik.touched?.form?.memberInfo &&
                memberInfo.length === 0 && { border: 'danger' })}
            >
              <Card.Header
